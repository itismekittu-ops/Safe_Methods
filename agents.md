...
| :--- | :--- | :--- | :--- |
| **00. Constitution** | `specs/00-Project_Constitution.md` | Core principles, data ownership directives, Mission, Vision. | Prior to making high-level tech stack or architectural decisions. |
| **01. PRD** | `specs/01-prd.md` | User stories (F1–F5), Get Quotes, Welcome email, Calendly integration, Consultant Bid Portal, Admin Review Gate, 5-Day SLA Dispatch. | Before implementing UI flows, features, or frontend user interactions. |
| **02. Architecture (ADD)** | `specs/02-architecture.md` | Tech stack, DB models (`consultant_bids`, `quote_requests`, `consultations`), Edge Functions, Calendly webhooks, Zoho SMTP relay architecture. | Before creating DB tables, writing Edge Functions, or building APIs. |
| **03. Business Rules** | `specs/03-business-rules.md` | Customer ownership rules (BR-CUST), Sensitive PII definitions (BR-PRIV), and Routing/SLA/Calendly rules (BR-ROUT). | When setting up form validation, data persistence, or user rights endpoints. |
