...
export function ExitIntentPopup() {
  ...
  return <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200" onClick={dismiss}>
      <div className="bg-surface border border-border-subtle rounded-xl shadow-raised w-full max-w-md relative" onClick={(e) => e.stopPropagation()}>
        <button onClick={dismiss} className="absolute top-4 right-4 p-2 text-muted-foreground hover:text-foreground hover:bg-muted rounded-md transition-colors" aria-label="Close">
        ...
        {done ? <div className="p-8 text-center">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <TextInput type="email" label="Email" placeholder="you@example.com" value={email} onChange={(e) => setEmail(e.target.value)} />
              {error && <p className="text-sm text-destructive bg-destructive/10 border border-destructive/30 rounded-md px-4 py-3">
                  {error}
                </p>}
              <Button type="submit" variant="primary" className="w-full bg-accent text-primary hover:bg-accent/90 border-transparent" disabled={submitting}>
                {submitting ? "Subscribing..." : "Get Free Rate Updates"}
                {!submitting && <ArrowRightIcon className="w-4 h-4 ml-2" />}
              </Button>
            </form>

            <button onClick={dismiss} className="w-full text-center text-sm text-muted-foreground hover:text-foreground transition-colors mt-4">
              No thanks, I'll pass
            </button>
          </div>}
      </div>
