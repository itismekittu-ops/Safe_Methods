...
export function ResetPassword() {
  ...
  return <main className="container mx-auto px-4 py-16 md:py-24 flex justify-center">
      <div className="w-full max-w-md">
        ...
        <Card>
          <CardBody className="p-8">
            {success ? <div className="text-center space-y-5">
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  <TextInput label="New Password" type="password" placeholder="Enter new password" value={password} onChange={(e) => setPassword(e.target.value)} required />

                  <div className="space-y-1.5">
                    {STRENGTH_RULES.map((rule) => {
                  const passed = rule.test(password);
                  return <div key={rule.label} className="flex items-center gap-2">
                          <div className={`w-4 h-4 rounded-full flex items-center justify-center text-xs ${passed ? "bg-success/20 text-success" : "bg-muted text-muted-foreground"}`}>
                            {passed ? "✓" : ""}
                          </div>
                  ...
                  <TextInput label="Confirm Password" type="password" placeholder="Re-enter new password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required />
                  {confirmPassword.length > 0 && !passwordsMatch && <p className="text-xs text-destructive -mt-2">Passwords do not match.</p>}

                  {error && <p className="text-sm text-destructive bg-destructive/10 border border-destructive/30 rounded-md px-4 py-3">
                      {error}
                    </p>}

                  <Button type="submit" variant="primary" className="w-full bg-accent text-primary hover:bg-accent/90 border-transparent" disabled={submitting || !allRulesPassed || !passwordsMatch}>
                    {submitting ? <>
