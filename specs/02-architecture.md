...
* **Context-Injected Grounding & Interactive Top Matches Filtering:** The single source of truth for all rate data is the Supabase `rates` table. The Top Matches panel maintains reactive filter state (`category: 'loan' | 'investment' | 'mortgage'`, `rateType: 'variable' | 'fixed'`, `tenure: 1..5`). The panel dynamically evaluates these parameters against loaded database records to sort and rank the top 3 distinct institutions deterministically (ascending for borrowing/debts, descending for investment returns). Hardcoded percentage rates in UI templates are strictly prohibited.
...
* **`quote_requests`**: `(id, reference_id [TEXT UNIQUE, internal only], name, email, phone, request_type [loan/investment/mortgage], loan_amount, monthly_income, investment_amount, tenure, property_value, down_payment, combined_monthly_debt, selected_institutions[], consent_given, consent_timestamp, sla_deadline [TIMESTAMPTZ], aggregated_quotes_sent [BOOLEAN], aggregated_quotes_sent_at [TIMESTAMPTZ], status, created_at)`
