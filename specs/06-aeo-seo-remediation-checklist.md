...
**Audit finding this responds to:** the live site returned no server-rendered content to crawlers, and Google's index still shows stale content from a prior WordPress site describing an unrelated business.

**Update:** Phase 0 is complete, executed against the site's real route structure (`/`, `/services`, `/blog`) rather than the originally assumed route list — see the note under Phase 0 below and `05-aeo-seo-standards.md`'s "Actual Site Structure" section.
...
## Phase 0 — Critical Fixes (Do First) — ✅ COMPLETE

**Note:** The original checklist assumed routes (about, contact, how-it-works, rates, consultants, banks) that don't exist on the real site. The real site is a single-page structure — `/`, `/services`, `/blog` — per `05-aeo-seo-standards.md`'s "Actual Site Structure" section. Bolt correctly declined to invent the missing pages with placeholder content (per rule 4) and completed Phase 0 against the real route list instead. Items below reflect what was actually built.

- [x] 🤖 Implement SSR/static prerendering for the real public routes: `/`, `/services`, `/blog`. Chat/authenticated routes unaffected. *(`05` §3.1)* — Puppeteer-based prerender step added to the build; raw HTML fetch confirmed for all three routes.
- [x] 🤖 Add unique title + meta description + OG/Twitter tags per route, present in the prerendered HTML. *(`05` §3.2-3.3)* — implemented via `react-helmet-async`.
- [x] 🤖 Add `robots.txt` allowing crawl of all public routes, referencing the sitemap. *(`05` §3.5)*
- [x] 🤖 Add `sitemap.xml` auto-generated from the live route list. *(`05` §3.6)*

**Outstanding from Phase 0 (flagged by Bolt, needs your input, not Bolt's):**
- [ ] 🧑 Supply a real `og:image` (1200×630) — currently references `og-default.jpg`, which doesn't exist.
- [ ] 🧑 Decide whether `/services` and `/blog` should eventually become real listing/dynamic routes (`/services/:slug`, `/blog/:slug`) — noted as a future product decision, not part of this remediation.

**Gate check:** Raw HTTP fetch (`curl`, not DevTools) of `/`, `/services`, `/blog` returns real, unique content and metadata. **You should still verify this yourself** before treating Phase 0 as fully signed off — don't rely solely on Bolt's self-report.
