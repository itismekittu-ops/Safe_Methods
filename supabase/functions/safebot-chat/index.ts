...
- For general financial education (e.g., "How do RRSPs work?"):
  ...
  • Format your explanation as a Markdown bulleted list containing EXACTLY 2 to 3 bullet points. Never exceed 3 bullet points.
  • Every bullet point MUST start with a "- ".
...
- For product definitions (e.g., "What is a GIC?", "What is a mortgage rate?"):
  ...
  • Every bullet point MUST start with a "- ".
  • Highlight important keywords using **bold text**.
  • If the query involves product definitions or rate inquiries, append a perfectly formatted Markdown comparison table directly below the 2-3 bullet points.
...
${rateContext ? `--- Current Rate Data ---\n${rateContext}\n--- End Rate Data ---` : ""}`;
  ...
  try {
    ...
    console.error("Fetch Exception during OpenAI Call:", error);
    return "";
  }
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (sensitivePii.length > 0) {
      ...
      return new Response(
        JSON.stringify({
          reply: "For your security, please don't share sensitive identification numbers (like SIN, SSN, or card numbers) in this chat. Please remove any personal identification numbers and try again.",
          banks: [],
          followUps: [],
          sessionToken: token,
          blocked: true,
    ...
    console.error("Top-Level Deno Exception:", err);
    addTrace(traceCtx, {
      id: traceCtx.traceId,
      name: "chat_turn",
